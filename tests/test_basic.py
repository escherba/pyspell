...
DATA3 = {
    'computer': 'computar', 'mortgage': 'morgage'
}


def spelltest(corrector, tests, bias=None, verbose=False):
    import time
    n, bad, unknown, start = 0, 0, 0, time.clock()
    bias_map = Counter()
    if bias:
        for target in tests:
            bias_map[target] += bias
    corrector.bias = bias_map
    get_word_value = corrector.word_value
    for target, wrongs in tests.items():
        for wrong in wrongs.split():
            n += 1
            w = corrector.correct(wrong)
            if w != target:
                bad += 1
                unknown += (target not in corrector.nwords)
                if verbose:
                    LOG.info(
                        'correct(%r) => %r (%d); expected %r (%d)'
                        % (wrong, w, get_word_value(w), target, get_word_value(target))
                    )
    return dict(bad=bad, n=n, bias=bias, fraction=1.0 - bad / n,
...
class TestBasic(unittest.TestCase):
    ...
    def test_norvig(self):
        result = spelltest(self.corrector, DATA1, bias=100, verbose=True)
        self.assertGreaterEqual(result['fraction'], 0.70)

    def test_spam(self):
        result = spelltest(self.corrector, DATA3, bias=None)
        self.assertGreaterEqual(result['fraction'], 1.00)
