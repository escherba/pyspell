...
def evaluate(df, data_path, suggestions=0):
    ...
    df["observed"] = df["error"].apply(lambda x: pysp.correct(x, suggestions=suggestions))

    if suggestions > 0:
        df["TP"] = df.apply(lambda x: x["expected"] in x["observed"] and x["error"] not in x["observed"], axis=1)
        df["FN"] = df.apply(lambda x: x["expected"] not in x["observed"] and x["error"] not in x["observed"], axis=1)
        df["FP"] = df.apply(lambda x: x["expected"] not in x["observed"] and x["error"] in x["observed"], axis=1)

        # suggested corrections contain the expected correction and not the error
        true_positives = df[df["TP"] is True]
        ...
        false_negatives = df[df["FN"] is True]
        # suggested corrections don't contain the expected correction but they do contain the error
        false_positives = df[df["FP"] is True]
    else:
        # error was corrected to the expected correction
        true_positives = df[df["expected"] == df["observed"]]
        # error was corrected to something else
        false_negatives = df[df["expected"] != df["observed"]]
        false_negatives = false_negatives[false_negatives["observed"] != false_negatives["error"]]
        # error was corrected (or rather, not corrected at all) to itself
        false_positives = df[df["observed"] == df["error"]]
...
def assemble_test_data(path_to_Birkbeck_subset):
    ...
    for line in tab0:
        ...
        df = df.append({"error": error, "expected": correction, "edit distance": None}, ignore_index=True)
